...
export default function MovieModal({ onAddMovie }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger className="flex items-center gap-2 bg-[#EA7317] hover:opacity-90 text-white px-4 py-2 rounded-full text-sm font-semibold shadow-sm transition-all active:scale-95">
        <Plus className="w-4 h-4" />
        Add Movie
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px] bg-[#73BFB8] text-white border-none shadow-2xl">
        <DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-2">
          <div className="flex flex-col gap-1.5">
            ...
            <Input
              ...
              className="bg-white/20 border-white/30 text-white placeholder:text-white/60 focus-visible:ring-white text-sm"
              required
            />
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Select value={formData.status} onValueChange={(val) => handleSelectChange("status", val)}>
              <SelectTrigger className="bg-white/20 border-white/30 text-white focus:ring-white text-sm">
                <SelectValue placeholder="Select status" />
              </SelectTrigger>
              <SelectContent className="bg-[#73BFB8] border-white/30 text-white">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-4 bg-[#EA7317] hover:opacity-90 text-white font-bold py-2.5 px-4 rounded-xl text-sm disabled:opacity-50 transition-all shadow-md active:scale-[0.98]"
          >
